...
SOURCE=ROOT/'assets/art/source/presentation-v3-fortress/v2'
DEST=ROOT/'assets/art/materials/presentation_v3/v2'
REVIEW=ROOT/'docs/reviews/presentation-v3/T-089/v2'
...
def bands():
    ...
    for tier,base in {'hot':(.25,.175,.095),'working':(.30,.31,.29),'cold':(.36,.42,.43)}.items():
        ...
        result[tier]={'albedo':f'res://assets/art/materials/presentation_v3/v2/band_{tier}_albedo.png','normal':f'res://assets/art/materials/presentation_v3/v2/band_{tier}_normal.png','emission':f'res://assets/art/materials/presentation_v3/v2/band_{tier}_emission.png','size':[w,h],'repeat_x':True,'edge_rgba_max_error':0,'normal_source':'explicit analytic deck height; periodic recesses and raised edge rails'}
    return result
...
def main():
    ...
    for index,k in enumerate(KINDS):
        ...
        assert len(unique)>=3,(k,'orientation root failed to rotate geometry',len(unique))
        if k in ('flak','lance_emitter','point_defense','repair_node','relay','tractor_lane'):
            assert len(unique)==12,(k,'directional model needs12 distinct bearing frames',len(unique))
        ...
        metadata['classes'][k]={c:f'res://assets/art/materials/presentation_v3/v2/{k}_{c}.png' for c in ('albedo','normal','emission','alpha')}
