...
OUT = ROOT/'assets/art/source/presentation-v3-fortress/v1'
SCENE = 'T089_Fortress_v1'
PFX = 'T089V1_'
...
def build_kind(k,c,m):
    ...
    rig=bpy.data.objects.new(PFX+k+'_bearing',None);c.objects.link(rig)
...
def render(k,start=0,end=12,channels=('albedo','normal','emission','beauty')):
    ...
    rig=bpy.data.objects[PFX+k+'_bearing']
...
def save_source():
    ...
    for k in KINDS:bpy.data.objects[PFX+k+'_bearing'].rotation_euler.z=0
    for k in KINDS:bpy.data.collections[PFX+k].hide_render=k!='flak'
    s.view_layers[0].update()
    path=OUT/'t089_fortress_v1.blend'
