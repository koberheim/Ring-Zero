...
# D-107: same scheduled-arrival shape as Tunneler/Foundry, but Transfer carries
# no stat multipliers - its distinct threat is entirely the one-time hop
# (implemented in LiveSimulation), not raw stats.
...
static func arrivals_between(profile: BalanceProfile, start_seconds: float, end_seconds: float) -> Dictionary:
	...
	var first := float(profile.value("transfer.first_arrival_seconds"))
	var interval := float(profile.value("transfer.arrival_interval_seconds"))
	if not is_finite(first) or not is_finite(interval) or interval <= 0:
		return _arrival_failure("Transfer schedule requires a finite positive interval")
	...
	if not is_finite(previous) or not is_finite(completed) or previous >= INTEGER_LIMIT_EXCLUSIVE or completed >= INTEGER_LIMIT_EXCLUSIVE:
		return _arrival_failure("Transfer arrival counts exceed supported signed integer range")
	var previous_count := int(previous)
	if previous_count == MAX_INTEGER:
		return _arrival_failure("Next Transfer sequence exceeds supported signed integer range")
...
static func spawn_descriptor(profile: BalanceProfile, sequence: int, outer_ring: int) -> Dictionary:
	...
	var due := float(profile.value("transfer.first_arrival_seconds")) + float(sequence - 1) * float(profile.value("transfer.arrival_interval_seconds"))
	if not is_finite(due):
		return _failure("Transfer due time must be finite")
	...
	return {"ok": true, "spawn": {
		"sequence": sequence, "elapsed_seconds": due, "kind": &"transfer", "hopped": false,
