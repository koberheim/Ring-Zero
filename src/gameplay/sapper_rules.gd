class_name SapperRules
extends RefCounted

# D-108: same scheduled-arrival shape as Tunneler/Foundry/Transfer, but Sapper
# carries no stat multipliers - its distinct threat is entirely in what its
# damage applies to (relay_hp instead of wedge HP), implemented in
# LiveSimulation, not raw stats. Movement and wall interaction are completely
# ordinary, per D-108 Option A.
...
static func arrivals_between(profile: BalanceProfile, start_seconds: float, end_seconds: float) -> Dictionary:
	...
	var first := float(profile.value("sapper.first_arrival_seconds"))
	var interval := float(profile.value("sapper.arrival_interval_seconds"))
	if not is_finite(first) or not is_finite(interval) or interval <= 0:
		return _arrival_failure("Sapper schedule requires a finite positive interval")
	...
	if not is_finite(previous) or not is_finite(completed) or previous >= INTEGER_LIMIT_EXCLUSIVE or completed >= INTEGER_LIMIT_EXCLUSIVE:
		return _arrival_failure("Sapper arrival counts exceed supported signed integer range")
	var previous_count := int(previous)
	if previous_count == MAX_INTEGER:
		return _arrival_failure("Next Sapper sequence exceeds supported signed integer range")
...
static func spawn_descriptor(profile: BalanceProfile, sequence: int, outer_ring: int) -> Dictionary:
	...
	var due := float(profile.value("sapper.first_arrival_seconds")) + float(sequence - 1) * float(profile.value("sapper.arrival_interval_seconds"))
	if not is_finite(due):
		return _failure("Sapper due time must be finite")
	...
	return {"ok": true, "spawn": {
		"sequence": sequence, "elapsed_seconds": due, "kind": &"sapper",
