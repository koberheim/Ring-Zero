...
| T-078 | Art pipeline stage 0-1: anchor and vertical slice | Kevin (generation), Astra (review) | 🟡 In progress | **Anchor step done (D-125):** `assets/art/anchor/anchor_approved.png`. Took two rounds — the first 5 candidates nailed wear/palette/detail but came back at a hero-shot camera angle instead of near-overhead; `docs/art-prompts/v1-templates.md`'s object-camera phrasing was tightened (explicit front-face/top-face ratio, concrete negatives) and the approved design regenerated. Result was a real partial fix: the mounting base now reads as genuine flat top-down, but the mass driver barrels still show more length than the ratio calls for (~30-35° apparent angle, not a true 20°) — accepted as this asset's shape (a projecting weapon inherently shows more length than a squat object would) rather than pushed through a third correction round. Remaining: the five-asset slice (band strip, decal sheet, mount, one weapon head, standard machine) referencing this anchor, normalised, and passed through the in-game review gate together. Real decision point: if the slice is coherent in engine, the remaining ~30 assets are mechanical; if not, prompt templates change before more budget is spent. Process in `docs/art-t078-brief.md` and `docs/art-prompts/v1-templates.md`. **D-124 note:** style/mood reference is already validated by the 9 approved mockups (`docs/reviews/artifacts/mockups-2026-09-08/`) — this task is about the isolated/flat/normalised production pipeline, not re-litigating direction. Canonical star for this pass is yellow. Standard-machine asset must be checked hard against the emissive-is-rank negative constraint before acceptance — it failed on the first mockup attempt. |
| T-079 | Art pipeline stages 2-4: full asset pass | Kevin (generation), Astra (review) | ⚪ Not started | Remaining band tiers and damage sheet, the other 12 building heads, then 5 elites and the Assembler. Every asset through the same normalisation steps and the same four-point review gate (close zoom, strategic zoom, against every star skin, and in a crowd for machines). Blocked on T-078's slice passing. |
