...
**Updated:** 2026-09-09. **Current review:** Full47/47 checkpoint and independent collapse40/40, readability2690/2690 and run-memory24/24 pass. D implements full theme/native layout; E produces new versioned structural Blender assets. B profiles the retained dense-collapse111ms spike on an isolated source. Canonical timing and bounded collapse pass, but dense stress, human gates and source audio remain open. Zero admitted audio assets; no release promotion.
...
|---|---|---|---|---|
| T-081 | Renderer, lighting and cache performance proof | Sol | 🟠 In review - technical slice accepted | Mobile default; full44/44, independent41/41 and three clean matched Mobile samples pass. Reviewer median16.679/p9517.120/max19.014 ms. Historical GL/control failures retained. [Acceptance](docs/reviews/presentation-v3/T-081/reviewer-acceptance.md); final integrated/human gates pending. |
...
| T-083 | Ring collapse vertical slice and catastrophe gate | Luna | 🟡 In progress - early interface accepted, stress repair open | Source67a49cb; native40/40 independently, full47/47. Timestamped RC2/A/new/reduced/off/blind clips retained. Canonical samples pass; bounded25-hardware collapse max19.361ms. Dense120-hardware collapse max111.127ms FAIL; B profiles cause. Human/sound/final-core flow pending. [Review](docs/reviews/presentation-v3/T-083/reviewer-early-acceptance.md). |
...
| T-085 | Free online AI audio, score and event mix | Luna | 🟡 In progress - adapter accepted, sources blocked | Audio84/84 and full44/44 pass; 52 cue families, zero admitted files. Concrete free ZeroGPU quota exhausted until approximately September10 03:46 UTC. Production tooling14/14 preparation checks; [handoff](docs/reviews/presentation-v3/T-085/audio-interface-handoff.md). B/D wiring and native/human listening pending; | T-085 | Free online AI audio, score and event mix | Luna | 🟡 In progress — source diagnosis/audio adapter | Stream C; [pilot report](docs/reviews/presentation-v3/T-085/ace-pilot-attempt.md). One anonymous 20-second FLAC request accepted, then `error: null`; no output or spending. Model/terms/request evidence retained. Stable Audio commercial eligibility/host applicability remains unresolved. | spent. |
| T-086 | CheckButton quick win and complete instrument theme | Luna | 🟡 In progress - full D implementation | Early slice7a85a88 remains accepted. Full industrial tokens/control states now being implemented under the exclusive D native-layout lease; [plan](docs/reviews/presentation-v3/T-086/full-theme-preparation.md). Final theme/native checks pending. |
| T-087 | Responsive native composition and correct map framing | Luna | 🟡 In progress - exclusive D lease | A/B interfaces reviewed;72 native before frames retained. Implement actual native stage, usable-map rectangle, input/caches, compact/normal/wide and independent saved audio levels. [Contract](docs/contracts/T-087-responsive-layout.md). |
| T-088 | Start, HUD, pause, settings, controls and outcome composition | Luna | 🟡 In progress - isolated memory accepted | Helper4704c1d independently24/24; truthful bounded actual samples and image identity. Shared screen integration follows D layout/E interface review; no natural-run or image capture claim yet. |
| T-089 | Fortress state, structural depth and remaining T-079 asset failures | Terra | 🟡 In progress - versioned asset production | Exact20-subject audit retains original failures. E now prepares new fixed-camera Blender bearing assets; old shipping art untouched pending D handoff. [Audit](docs/reviews/presentation-v3/T-089/preparation-disposition.md). |
| T-090 | Swarm LOD and elite visibility at strategic zoom | Terra | 🟡 In progress - pure interface accepted | f97dcf7; independent2690/2690 with full47/47. Actor conservation, density, hysteresis and physical-floor projection prepared; live batching/F atlas integration and human/native stress remain pending. |
| T-091 | Readable ring integrity and power instrument | Terra | 🟡 In progress - pure interface accepted | f97dcf7 faithful fresh status and exact radar geometry verified within2690 checks;232px unit-scale minimum for12bands. Native instrument/style integration follows D; human checks pending. |
| T-092 | Mechanic-led elite and Assembler art | Luna | 🟠 In review - provisional assets accepted | Six detailed Blender mechanisms,8poses each,24 aligned relightable atlases. Root inspected forms/maps and independently verified281hashes. Initial static motion/edge defects repaired with failure evidence retained. [Handoff](docs/reviews/presentation-v3/T-092/v2/HANDOFF.md). E native/crowd integration and human recognition remain pending. |
...
| T-079 | Art pipeline stages 2-4: full asset pass | Kevin (generation), Astra (review) | 🟠 In review | **All 20 remaining assets generated, normalised and reviewed.** Full honest report at `docs/reviews/T-079-production-assets.md`, 32 evidence captures at `docs/reviews/artifacts/T-079-*.png` — verified directly against the images, not taken on the report's word alone. **Verdict: production gate does not pass**, correctly not fudged. Confirmed real findings: (1) referencing the anchor pulled its literal octagonal mount SHAPE, not just material, into Wall/Tractor Lane/Occlusion Screen — all three read as turret pedestals instead of a barrier/marking/screen; (2) the damage sheet's motifs carry unwanted plate backing instead of being isolated decals like T-078's working decal sheet; (3) Tunneler/Transfer/Breacher share a silhouette family and are hard to tell apart even at full close-up size — Sapper and Foundry are genuinely distinct, proving the approach works; (4) Relay/Repair Node/Tractor Lane/Sapper show an actually-observed lean/off-axis problem in the flat rotation dial, not just "unverified pending live renderer"; (5) Point Defense isn't smaller/lighter than Flak as required. Bands, damage motifs (content, not framing), Debris Field, Armor Plating, Foundry, Sapper, and the emissive rule on all 6 elite/Assembler assets all passed cleanly. **Two real decisions came out of review, not just art fixes:** D-127 (Wall becomes a radially-outward-facing barrier, not a turret-pedestal object; whether Wall is restricted to the outermost ring is a separate open TBD) and D-128 (each elite gets its own cold-family accent colour — cyan/blue/teal/violet/icy-white — superseding the single shared cyan accent; Assembler keeps a larger icy-white, not a 6th colour). Templates for Wall (§17) and elites/Assembler (§21-26) in `v1-templates.md` are updated for both. **Corrective regeneration pass (Wall, Tractor Lane, Occlusion Screen mount-bleed; damage sheet isolation; Tunneler/Transfer/Breacher differentiation; Point Defense sizing) is explicitly flagged TBD by Kevin — not authorized yet, do not launch it without a further go-ahead.** D-128 also raised an unresolved accessibility tension (colour-only differentiation between silhouette-identical elites vs. D-115's non-colour-only commitment) that needs an answer before shipping, independent of the TBD regeneration timing. |
