...
func _run() -> void:
	...
	check(PCSettings.rebind("Tactical overlay", KEY_W) == ERR_INVALID_PARAMETER and PCSettings.bindings == PCSettings.DEFAULT_KEYS, "Modifier-incompatible swaps are rejected without corrupting bindings")
	check(PCSettings.rebind("Tactical overlay", KEY_F2) == OK, "Tactical overlay can move to an unassigned ordinary key")
	check(PCSettings.rebind("Tactical overlay", KEY_ALT) == OK, "Tactical overlay can return to Alt")
	check(PCSettings.rebind_button("invalid", "Place / select", MOUSE_BUTTON_RIGHT) == ERR_INVALID_PARAMETER and PCSettings.mouse == PCSettings.DEFAULT_MOUSE, "An invalid button group cannot mutate mouse settings")
